...
public class MiniHBaseCluster implements HConstants {
   ...
   * 
   * @param conf
   *          HBaseConfiguration to be used for cluster
   * @param numRegionServers
   *          initial number of region servers to start.
   * @throws IOException
   */
    ...
    try {
      ...
      while (true) {
        try {
          ...
          // this port is already in use. try to use another (for multiple
          // testing)
  ...
  public String startRegionServer() throws IOException {
    LocalHBaseCluster.RegionServerThread t = this.hbaseCluster
        .addRegionServer();
    t.start();
   ...
   *          regionserver's shutdown. Usually we do but you do not want to do
   *          this if you are running multiple regionservers in a test and you
   *          shut down one before end of the test.
   * @return the region server that was stopped
   */
  public LocalHBaseCluster.RegionServerThread stopRegionServer(
      int serverNumber, final boolean shutdownFS) {
    LocalHBaseCluster.RegionServerThread server = hbaseCluster
  ...
  public void flushcache() throws IOException {
    for (LocalHBaseCluster.RegionServerThread t : this.hbaseCluster
        .getRegionServers()) {
